...
class WP {
	...
	public function parse_request($extra_query_vars = '') {
		...
		if ( ! empty($rewrite) ) {
			...
			$pathinfo = isset( $_SERVER['PATH_INFO'] ) ? urldecode($_SERVER['PATH_INFO']) : '';
			if (mb_detect_encoding($pathinfo, 'latin1,UTF-8,GBK') == 'GBK') {
				$pathinfo = mb_convert_encoding($pathinfo, 'GBK', 'UTF-8');
			}
			...
			if (mb_detect_encoding($pathinfo, 'latin1,UTF-8,GBK') == 'GBK') {
				$req_uri = mb_convert_encoding($req_uri, 'GBK', 'UTF-8');
			}
